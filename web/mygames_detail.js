...
var level       = 1;
var previousMove = "";
lozData.page    = 'mygames_detail.htm';
lozData.idInfo  = '#info';
lozData.gameInfo = '#gameInfo';
...
var onDrop = function(source, target, piece, newPos, oldPos, orientation) {
  ...
  previousMove = chess.fen();
  var move = chess.move({from: source, to: target, promotion: $('#promotepeace').val()})
  console.log(move)
  if (!move) {
    //console.log('invalid');
    return 'snapback';
...
$(function() {
  ...
  $('#resign').click(function(){
    alert('you lose!');
    $(lozData.idInfo).prepend('You lose' + '<br>');
  ...
  $.get( "http://localhost:5001/games/id/"+gameId, function( data ) {

    $(lozData.gameInfo).append('Event:' + data.Event+ '<br>');
    $(lozData.gameInfo).append('Black:' + data.Black + '('+ data.BlackElo+')' + '<br>');
    $(lozData.gameInfo).append('White:' +data.White+ '('+ data.WhiteElo+')' + '<br>');
    $(lozData.gameInfo).append('Result:' + data.Result+ '<br>');
    $(lozData.gameInfo).append('Game Date:' + data.Date+ '<br>');
    ...
    data.Moves.forEach( function (move) {
      ...
      $('#moves').html(chess.pgn({newline_char: '<br>'}));
      $('#mymoves').append(   ' <a id="'+move.fen+'" onclick=loadFen(this)>' + move.san +'</a>'  );
      if (i%2==0){
        $('#mymoves').append("</br>");
      }
      i++;
